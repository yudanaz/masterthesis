...
public:
	...
	//set a new image from open cv Mat object
	void update_image_ycbcr(Mat image);
	...
	//transform a UINT* image into open cv mat
	Mat getMat(UINT* img_uint);

	//transform a open cv mat into UINT* image
	UINT* getUINT(Mat mat);
